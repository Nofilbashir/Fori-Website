  ...
  const handleClick=()=>{
   ...
   return 0
    ...
    return(
      ...
      <NavLink  to='/signup' className="signUp_link" onClick={()=>{dispatch(userAdd(false))}} >{useradd?<span className='login-logout'>Logout</span>:<span className='login-logout'>Sign Up</span>}</NavLink>
      ...
      {useradd?<span className='User_Avatar'>{Uname.toUpperCase().slice(0,1)}</span>:<NavLink  to='/login' className="signUp_link" ><span className='login-logout'>Log In</span></NavLink> }
        </div>
